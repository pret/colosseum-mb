...
s32 sub_020023E8(void)
{
    ...
    while (TRUE)
    {
        ...
        if (keys & A_BUTTON)
        {
            // Nonmatching part is here. gUnknown_02021860 pointer is not loaded into register, but reused from r8.
            // In a way, compiler is slighly smarter with my function than originally. I'm not sure where the difference could be in the code however.
...
#else
s32 NAKED sub_020023E8(void)
{
    asm_unified(".equ SE_SELECT,  0\t\n\
	.equ SE_FAILURE, 1\t\n\
                push {r4, r5, r6, r7, lr}\t\n\
	mov r7, sl\t\n\
	mov r6, sb\t\n\
